"""
extract/臺北市宗教社會服務概況.py
====================
從 tsis.dbas.gov.taipei 下載臺北市外籍移工人數時間數列統計資料 CSV。
...
_URL = (
    "https://tsis.dbas.gov.taipei/statis/webMain.aspx?sys=220&ymf=8400&kind=21&type=0&funid=a05024801&cycle=4&outmode=12&compmode=0&outkind=3&deflst=2&nzo=1"
)
...
def extract(config: dict) -> pd.DataFrame:
    ...
    print(f"[Extract] 外籍移工：共 {len(df)} 筆，欄位：{list(df.columns)}")
    return df
