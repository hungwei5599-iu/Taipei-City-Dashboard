"""
extract/臺北市宗教概況(寺廟、教會(堂)及信徒人數).py
====================
從 tsis.dbas.gov.taipei 下載臺北市宗教概況(寺廟、教會(堂)及信徒人數)統計資料 CSV。
...
_URL = (
    "https://tsis.dbas.gov.taipei/statis/webMain.aspx?sys=220&ymf=5700&kind=21&type=0&funid=a05024701&cycle=4&outmode=12&compmode=0&outkind=1&deflst=2&nzo=1"
)
...
def extract(config: dict) -> pd.DataFrame:
    ...
    print(f"[Extract] 臺北市宗教概況(寺廟、教會(堂)及信徒人數)：共 {len(df)} 筆，欄位：{list(df.columns)}")
    return df
