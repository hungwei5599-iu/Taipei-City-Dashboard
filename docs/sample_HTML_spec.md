...
## 21. ChatBox Core HTML Response

ChatBox 回答可以帶一段核心 HTML，但只放使用者真正需要先看的資訊：目標名稱、摘要、距離、待診人數、等待時間，以及跳轉地圖連結。地圖上的開組件、標點、畫線、畫範圍、zoom in、popup 仍由 `ui_actions` 執行，不需要在聊天泡泡裡再產生大型卡片/表格組件。

```json
{
  "answer": "我已整理最近的急診結果，地圖可顯示候選點、候選範圍、連線與資訊卡。",
  "html": "<section class=\"guide-html-core\"><h4>亞東</h4><p>我已整理最近的急診結果。地圖可顯示候選點、候選範圍、你的位置到最近醫院的連線，並縮放到資訊卡。</p><div class=\"guide-html-metrics\"><div><span>距離</span><strong>2.3 公里</strong></div><div><span>待診</span><strong>4</strong></div><div><span>等待</span><strong>13 分鐘</strong></div></div><a class=\"guide-html-action\" href=\"/mapview?city=metrotaipei\">查看地圖</a></section>",
  "ui_actions": [
    { "type": "component.open", "payload": { "componentIndex": "Component2_er_ready", "title": "急診即時概況" } },
    { "type": "map.add_points", "payload": { "layerId": "ai-guide-Component2_er_ready-points" } },
    { "type": "map.add_polygon", "payload": { "layerId": "ai-guide-Component2_er_ready-bounds" } },
    { "type": "map.add_line", "payload": { "layerId": "ai-guide-Component2_er_ready-route" } },
    { "type": "map.fly_to", "payload": { "center": [121.4526, 24.9967], "zoom": 15.5 } },
    { "type": "map.open_card", "payload": { "title": "亞東", "coordinate": [121.4526, 24.9967] } }
  ]
}
```
