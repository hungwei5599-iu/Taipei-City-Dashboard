...
const initMap = () => {
	...
	map.on('load', () => {
		map.addSource('aed-isochrone', {
			type: 'geojson',
			data: { type: 'FeatureCollection', features: [] }
		});
    
		map.addLayer({
			id: 'aed-isochrone-layer',
			type: 'fill',
			source: 'aed-isochrone',
			paint: {
				'fill-color': '#E81123',
				'fill-opacity': 0.3
			}
...
const updateMapData = (geojsonData) => {
	if (map && map.getSource('aed-isochrone')) {
		map.getSource('aed-isochrone').setData(geojsonData);
	}
};
		...
		setTimeout(() => {
			const mockGeoJSON = {
				...
				features: [{
					...
					geometry: {
						type: 'Polygon',
						coordinates: [[[121.5654, 25.0330], [121.5854, 25.0330], [121.5854, 25.0530], [121.5654, 25.0530], [121.5654, 25.0330]]]
					}
				}]
			};
...
const getAiInsights = async () => {
	...
	try {
		const response = await axios.post('/api/v1/ai/chat/twai', {
			tool_name: 'query_aed_locations',
			city: currentCity.value,
			user_query: '請幫我分析 AED 設備的分佈與救護覆蓋率'
		});
...
<template>
  <div class="custom-component-container">
    ...
    <div class="visualization-area">
      ...
      <div
        v-if="mapError"
        class="map-fallback aed-fallback"
      >
...
.map-fallback { position: relative; display: grid; place-items: center; width: 100%; height: 350px; overflow: hidden; border: 1px solid #333; border-radius: 4px; background: radial-gradient(circle at 50% 45%, #482629 0, #351f22 38%, #211719 68%, #111 100%); }
.fallback-map-shape { position: relative; width: min(58%, 360px); aspect-ratio: 1.25; }
.fallback-zone { position: absolute; display: block; border: 2px solid rgba(255,255,255,0.45); background: rgba(232,17,35,0.24); }
...
.ai-panel h4 { margin: 0 0 8px 0; color: #E81123; }
